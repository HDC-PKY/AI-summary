...
### 진행 현황
- [x] 정책 로더/검증 통합하여 런타임 적용 (`src/core/data_pipeline/policies/engine.py` → `_load_policy_engine` 경유)
- [x] 정책 기반 인덱서(스캔/증분) 동작 추가 (`infopilot.py` scan/train/watch 경로, watcher 필터 개선)
- [x] 작업 스케줄러·모델 매니저 초기 버전 구현
- [x] 통합 테스트/로깅 보강 및 문서화

### 산출물
- 정책 엔진: `src/core/data_pipeline/policies/engine.py`
- 정책 로더 확장: `src/core/data_pipeline/policies/loader/__init__.py`
