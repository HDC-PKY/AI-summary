...
### 5.2 향후 확장 후보 (보류)
- 회의/STT, 사진 태깅 등은 아이디어 단계로 유지하되, 현재 저장소에서는 제외됨

## 6) 단계별 로드맵
- **P0: Core + 스마트 폴더** — 정책 엔진, 폴더별 인덱싱/보안/주기, 기초 검색
- **P1: 지식·검색 고도화** — 요약/Q&A, 출처 링크, 작업 센터
- ~~P2: 회의 비서 MVP~~ *(중단됨 — 지식 비서 집중)*
- ~~P3: 사진 비서 MVP~~ *(중단됨 — 지식 비서 집중)*
- **P2 (개편): 하이브리드/관리** — 클라우드 오프로딩, 감사/권한, 모델 자동 선택
...
- ~~**Cycle 3 – P2 (회의 비서 MVP)**~~ *(보류)
4. ~~**P2 – 회의 비서 MVP**~~
   - STT 파이프라인 연결, 사후 요약과 액션 아이템 추출 *(현재 저장소에는 포함되지 않음)*
    - 프로젝트/폴더 자동 저장 및 폴더 정책 연동 테스트 *(현재 저장소에는 포함되지 않음)*
    - 자료 정리: `docs/cycles/cycle_3.md`에 STT 품질/액션 아이템 검증 결과 기록 *(보존용 문서)*
- ~~**Cycle 4 – P3 (사진 비서 MVP)**~~ *(보류)
5. ~~**P3 – 사진 비서 MVP**~~
   - 이미지 메타/임베딩 추출, 중복 탐지·베스트샷 추천 알고리즘 구현 *(현재 저장소에는 포함되지 않음)*
    - GPU/CPU 선택 정책 및 캐시 관리 정비 *(현재 저장소에는 포함되지 않음)*
    - 자료 정리: `docs/cycles/cycle_4.md`에 모델 성능 및 리소스 통계 정리 *(보존용 문서)*
